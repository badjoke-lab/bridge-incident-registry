...
- reduce `risky_host_unarchived` ceiling from 29 to 26.
...
Risky-host unarchived unique URLs    26
...
3. change only the three reviewed archive fields and the risky-host ceiling;
4. run canonical validation, enum validation, full-corpus audit, exact source-count audit, source-quality audit, type/Astro check, and static build;

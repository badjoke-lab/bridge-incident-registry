# Taiko and Everclear candidate intake — 2026-07-25
...
Bridge Incident Registry record expansion remains paused until the seven-step public-consistency remediation is complete. This document preserves two time-sensitive, evidence-backed candidates without changing:
...
Neither candidate is approved for canonical promotion by this document. IDs, entity boundaries, classifications, loss fields, recovery fields, and relationship fields must be derived again from current canonical data after the remediation hold is lifted.
...
- Sophon, ZERO Network, Syndicate Labs, Over Protocol, and Mint Chain: chain or infrastructure shutdowns do not become BIR records without bridge-specific lifecycle or incident evidence.
...
3. decide Taiko and Everclear entity boundaries
4. obtain archived primary posts and any final postmortem / shutdown documentation
5. prepare a dedicated reviewed scope PR
6. assign IDs only from the then-current canonical maximums
7. promote through a separate canonical-data PR after required checks pass
```

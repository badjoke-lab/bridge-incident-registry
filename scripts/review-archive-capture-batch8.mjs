...
const outputPath = "docs/audits/phase3-archive-capture-batch8-candidate-verification.json";
...
async function verifyCandidate(candidate) {
  ...
  try {
    const cdx = await cdxSnapshots(candidate.url);
    for (const snapshot of cdx) {
      if (!discoveries.some((item) => item.archived_url === snapshot.archived_url)) discoveries.push(snapshot);
    }
  } catch (error) {
  ...
  for (const snapshot of discoveries.slice(0, 8)) {
    try {
...
const report = {
  ...
  policy: {
    ...
    retry_policy: "20-second requests and up to three replay attempts per capture",

...
if (mismatches.length !== 37) throw new Error(`Expected 37 mismatches, received ${mismatches.length}`);
...
const lines = ["# Source-count review Batch 2 inventory", "", "Status: temporary generated inventory", "", `Remaining mismatch baseline: ${mismatches.length}`, "", "## Targets", "", "```text", ...targets.map((event) => event.id), "```", ""];
...
fs.writeFileSync(path.join(root, "docs/audits/.source-count-review-batch2.md"), `${lines.join("\n")}\n`);
